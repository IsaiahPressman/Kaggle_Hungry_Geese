    ...
    def __call__(self, obs: Observation, conf: Configuration) -> str:
        ...
        if obs.step >= 1:
            ...
            overall_kl_divs = kl_divergence(
                self.all_actions[:obs.step],
                self.all_policy_preds[:obs.step],
                dim=-1,
                keepdim=True
            ) / (entropy(self.all_action_masks[:obs.step], dim=-1, keepdim=True) + eps)
            overall_kl_divs = overall_kl_divs.mean(dim=0)
            recent_kl_divs = kl_divergence(
                self.all_actions[obs.step-1],
                self.all_policy_preds[obs.step-1],
                dim=-1,
                keepdim=True
            ) / (entropy(self.all_action_masks[obs.step-1], dim=-1, keepdim=True) + eps)
            print(f'Overall KL-divergences: {print_array_one_line(overall_kl_divs.numpy())}', end=' ')
            print(f'Last step KL-divergences: {print_array_one_line(recent_kl_divs.numpy())}', end=' ')
