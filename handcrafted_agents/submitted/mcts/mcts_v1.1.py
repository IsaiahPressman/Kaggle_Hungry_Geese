...
class LightweightEnv:
    ...
    def canonical_string_repr(self, include_food: bool = True) -> str:
        if self.done:
...
def critic_func(state):
    ...
    l_mod = l_mod_func(state[0]['observation']['step'])
...
class Agent:
    def __init__(self, obs: Observation, conf: Configuration):
        ...
        self.expected_end_step = conf.episode_steps - 10.
    ...
    def __call__(self, obs: Observation, conf: Configuration):
        ...
        max_time = 0.98 + (max(obs.remaining_overage_time - 1., 0.) / max(self.expected_end_step - obs.step, 1.))
