...
class Agent:
    ...
    def compute_next_position(self, obs: Observation, conf: Configuration):
        ...
        for _ in range(len(remaining_food)):
            ...
            for goose_idx in priority_goose_indices:
                ...
                if len(selected_path) - 1 < len(goose):
                    for i, segment in list(enumerate(goose))[:(len(goose) - len(selected_path) + 2)]:
    ...
    def a_star(self, start_node: Node, goal_locs: List[tuple], occupied_matrix,
               max_depth=10) -> Tuple[List[Tuple[int, int]], bool]:
        ...
        while len(not_visited) > 0:
            ...
            if current_node.position in goal_locs:
                print(f'Depth/nodes: {max([v.g for v in visited])}/{len(visited)}', end=' ')
    ...
    def would_collide(self, goose_indices: Sequence[int]) -> List[bool]:
        # If 1 or more geese would remain after the collision, the shorter goose would like to collide
        # In case of ties in this case, one of the geese randomly opts for collision
        # If 0 geese would remain after the collision, the longer goose would like to collide
        ...
        all_goose_lengths = np.array([len(g) for g in self.geese])
        selected_geese_mask = np.array([i in goose_indices for i in range(len(self.geese))])
        colliding_goose_lengths = all_goose_lengths[selected_geese_mask]
        remaining_goose_lengths = all_goose_lengths[~selected_geese_mask]
        remaining_goose_lengths = remaining_goose_lengths[remaining_goose_lengths > 0]
        if num_geese_after_collision >= 1:
            would_collide = []
            if colliding_goose_lengths.max() < remaining_goose_lengths.min():
                for goose_length in colliding_goose_lengths:
                    would_collide.append(goose_length == colliding_goose_lengths.max())
            else:
                for goose_length in colliding_goose_lengths:
                    would_collide.append(goose_length < colliding_goose_lengths.max())
            if np.array(would_collide).sum() > 1:
                would_collide_idx = np.random.choice(len(would_collide),
                                                     p=np.array(would_collide).astype(np.float) / np.array(would_collide).sum())
                return [i == would_collide_idx for i in range(len(would_collide))]
            else:
