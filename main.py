...
EVAL_TIME = datetime.strptime("2021-03-23 2:35:00", "%Y-%m-%d %H:%M:%S")
...
@jit(state_type(state_type, state_type), **JIT_ARGS)
...
@jit(int32[:](state_type, tree_type, int32[:]), **JIT_ARGS)
def explore_moves(state, tree, moves):
...
class Agent:
    ...
    def __call__(self, obs, config):
        ...
        # Take weighted mean of NN prediction and search outcome
        moves = (self.tree.visits[0, self.state.index] / self.tree.visits[0, self.state.index].sum())
        eps = 1e-6
        # The log_sum_exp trick is probably unnecessary, but used anyway
        # This is the same as:
        # final_policy = (moves + eps) * (nn_policy + eps)
        final_policy = np.exp(np.log(moves + eps) + np.log(nn_policy + eps))
        ...
        print(
            ...
            f"Search/nn agree: {moves.argmax() == nn_policy.argmax()} - "
